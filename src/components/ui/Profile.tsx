...
function Tag({ stack }: { stack: Stack }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-1 sm:px-2.5 rounded text-[10px] sm:text-[11px] font-medium border transition-all ${
        stack.core
...
const linkBtnClass =
  "flex-1 min-h-[42px] flex items-center justify-center gap-2 px-3 py-2 rounded-md border border-white/10 bg-white/5 text-white/60 text-xs hover:border-white/20 hover:text-white transition-colors";
    ...
    <div className="w-full max-w-[420px] sm:max-w-[450px] lg:max-w-[340px] xl:max-w-[310px] mx-auto">
      <section className="relative w-full px-4 sm:px-6 pt-14 pb-6 rounded-2xl backdrop-blur-md bg-black/60 border border-white/10 shadow-2xl">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-md bg-[#04F7A4]/5 border border-[#04F7A4]/20 mb-5 w-full justify-center text-center">
            <span className="w-2 h-2 rounded-full bg-[#04F7A4] animate-pulse shrink-0" />
            <span className="text-[11px] sm:text-xs font-medium text-[#04F7A4]">
              Disponible — Valence, FR
            </span>
          </div>

          <div className="w-full border-t border-white/5 mb-4" />

          <p className="text-[10px] text-white/30 uppercase tracking-widest self-start mb-2.5">
            Réseaux
          </p>

          <div className="flex flex-col sm:flex-row gap-2.5 w-full mb-4">
            <a
          ...
          <p className="text-[10px] text-white/30 uppercase tracking-widest self-start mb-3">
            Stack
          </p>
          ...
          <div className="flex flex-col gap-3 w-full">
            {categories.map((cat) => (
              <div key={cat.label} className="w-full">
                <p className="text-[10px] text-white/30 uppercase tracking-widest mb-1.5 font-mono">
                  {cat.label}
                </p>
